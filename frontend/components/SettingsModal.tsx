// Settings window — opened from AccountMenu.
//
// Extension points intentionally left for later (not built — no visible
// stub either, per "don't build unless trivial"): default start location /
// home airport, "prefer step-free routes," export-my-data, light/dark
// theme, help & feedback, version/about. Travel preferences already lives
// in User.preferences (a generic JSON blob), so any of these just add a
// new key there without another migration.
...
export function SettingsModal({ visible, onClose }: Props) {
  ...
  // Re-seed everything whenever the modal (re)opens for the current user —
  // covers both a fresh open and switching accounts within one session.
  useEffect(() => {
      ...
      <View style={styles.card}>
        ...
        <ScrollView style={styles.body} contentContainerStyle={{ gap: spacing.lg }}>
          ...
          <View style={styles.section}>
            ...
            <Pressable
              onPress={() => { onClose(); router.push('/itineraries'); }}
...
const styles = StyleSheet.create({
  ...
  card: {
    width: 420, maxHeight: 620, backgroundColor: colors.surface, borderRadius: radius.lg,
